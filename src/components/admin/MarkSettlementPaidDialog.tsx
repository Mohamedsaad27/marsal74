...
import { PAYMENT_METHOD_OPTIONS, formatAmount, settlementPartyName } from "@/lib/admin/settlements-types";
...
export function MarkSettlementPaidDialog({ open, onOpenChange, item, onSave, loading = false }: Props) {
  ...
  return (
    <AdminDialogShell
      ...
      description="settlements — تسجيل طريقة الدفع والمرجع"
      icon={CircleDollarSign}
      badge={item.settlement_ref}
      tone="success"
      footer={
        <>
          <Button className="rounded-xl bg-success px-6 text-success-foreground hover:bg-success/90" onClick={handleSave} disabled={loading}>
    ...
    >
      <div className="space-y-4">
        <div className="rounded-2xl border border-border bg-muted/30 p-4 text-sm">
          ...
          <p className="mt-1">
            <span className="text-muted-foreground">net_amount:</span>{" "}
            <span className="text-lg font-bold tabular-nums text-success">{formatAmount(item.net_amount)} ج.م</span>
          </p>
        </div>
