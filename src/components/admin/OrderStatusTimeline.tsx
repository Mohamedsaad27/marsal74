            ...
            <li key={entry.id} className="relative flex gap-4 pb-6 last:pb-0">
              ...
              <span
                className={cn(
                  ...
                  isLatest && "border-success bg-success/10",
                )}
              >
                <span
                  className={cn("h-2 w-2 rounded-full", isLatest ? "bg-success" : "bg-primary")}
                />
              </span>
                ...
                <p className="mt-1 text-sm">
                  ...
                  {/* {entry.from_status != null && (
                    <span className="text-muted-foreground">
                      {" "}
                      — من {ORDER_STATUS_TO_KEY[entry.from_status]} إلى{" "}
                      {ORDER_STATUS_TO_KEY[entry.to_status]}
                    </span>
                  )} */}
                </p>
